...
namespace Valve.VR.InteractionSystem
{
	...
	public class Player : MonoBehaviour
	{
		...
		private float _tPadStart = 0;
		private float _tPadEnd = 0;
		//private float _tPadLength = 0;

		private float l_tPad_x = 0;
		private float l_tPad_y = 0;
		...
		private bool tPadHeld = false;

		public bool pingheld = false;
		...
		Vector2 delta = hands[1].GetComponent<Hand>().swipeDelta;
			SteamVR_Action_Boolean touch = hands[1].GetComponent<Hand>().touchPad;

			if (touch.GetState(SteamVR_Input_Sources.RightHand))
            {
				if(!tPadHeld && delta.x != 0)
                {
					_tPadStart = delta.x;
					tPadHeld = true;
                }
			...
			Debug.Log(tPadHeld);
			_tPadLength = _tPadEnd - _tPadStart;

			Debug.Log(_tPadLength);
			//Debug.Log(_tPadStart + " is start" + _tPadEnd + " is end");

			if (_tPadLength > 1.45f && !touch.GetState(SteamVR_Input_Sources.RightHand))
			{
				Debug.Log("RightTurn");
				transform.Rotate(0, 45, 0);
				ResetTrackPad();
			}
			else if (_tPadLength < -1.45f && !touch.GetState(SteamVR_Input_Sources.RightHand))
			{
				Debug.Log("LeftTurn");
				transform.Rotate(0, -45, 0);
				ResetTrackPad();
		...
		void PingSystem()
        {
			SteamVR_Action_Boolean tempPing = hands[0].GetComponent<Hand>().pingAction;
			if (tempPing.GetState(SteamVR_Input_Sources.LeftHand))
				pingheld = true;
			else
				pingheld = false;
        ...
        {
			float sprint = 1.5f;
			bool followHMD = true; //if true, move relative to headset, else, move in relative to left hand

			Vector2 delta = hands[0].GetComponent<Hand>().leftPos;
			//Debug.Log(delta);
			SteamVR_Action_Boolean touch = hands[0].GetComponent<Hand>().touchPad;
			
			//add sprint by pressing on pad/make teleport optional


			//Debug.Log(hmdTransform.eulerAngles);
			...
			if (followHMD)
            {
				noY = hmdTransform.forward;
				noX = hmdTransform.right;

            }
            else
            {
				noY = hands[0].transform.forward;
				noX = hands[0].transform.forward;
			}
			
			noY.y = 0;
			noX.y = 0;

			if (touch.GetState(SteamVR_Input_Sources.LeftHand))
			{

				transform.position += noY * Time.deltaTime * sprint  * delta.y;
				transform.position += noX * Time.deltaTime * delta.x;
