...
namespace Assets.Source.Trajectory
{
    ...
    public class ProjectileTrajectory: MonoBehaviour
    {
		...
		public void Awake(){
			target = GameObject.FindGameObjectWithTag ("Splastret");
		}
        public void Start()
        {

			initialValues.velocityInitial = holder.GetComponent<VelocityControl> ().velocity;
        ...
        public void Update()
        {
			//f = GameObject.FindGameObjectWithTag ("laser");
			//Vector3 d = new Vector3 (f.GetComponent<TrajectoryRays> ().rayz);
			//Debug.Log (self.GetComponent<TrajectoryRays> ().rayz);

//			if (points.trajectoryPoints.Count > 0) {


//				target.transform.position = points.trajectoryPoints [points.trajectoryPoints.Count - 1];
//				Debug.Log (points.trajectoryPoints[points.trajectoryPoints.Count - 1]);

//			}
			if (trajectoryLine.positionCount > 0) {
				endpoint = trajectoryLine.GetPosition (trajectoryLine.positionCount - 1);
				//Debug.Log (trajectoryLine.GetPosition (trajectoryLine.positionCount - 1));
				target.transform.position = trajectoryLine.GetPosition (trajectoryLine.positionCount - 1);
			}
			if (points.trajectoryPoints.Count > 0) {
	...
	public class TrajectoryRays//: MonoBehaviour
    {
		public LineRenderer lr;
		public Vector3 rayz;
		...
		public void Update(){
			...
			self = GameObject.FindGameObjectWithTag ("laser");
			lr1 = GameObject.FindGameObjectWithTag ("lr1");
			lrr = lr1.GetComponent<LineRenderer> ();
			lr = self.GetComponent<ProjectileTrajectory>().trajectoryLine;

			Vector3 gg = new Vector3 (0, 0, 0);
			lrr.SetPosition (0, gg);
			//lrr.SetPosition (1, rayz);
			//lrr.SetPosition (0, lr.GetPosition (lr.positionCount - 1));
		}
        ...
        public void castRaysFromPoints(List<Vector3> points, byte count, byte lastPointIndex)
        {
            Ray ray;
            RaycastHit hit;
            ...
            for (i = 0; i < count; i++)
            {
                if(i != lastPointIndex)
                {
                    ...
                    {
						if(hit.collider && hit.collider.tag != "porjo1")
                        { 
							...
							//Debug.Log (rayz + "is ray");
							rayz = RayHitPosition;
							if (RayHitPosition != null) {
								//rayz = RayHitPosition;
								//lrr.SetPosition (0, rayz);
							}
							//target.transform.position = trajectoryLine.GetPosition (trajectoryLine.positionCount - 1);
